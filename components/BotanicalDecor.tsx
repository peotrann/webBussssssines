...
const FloatingElement = ({ delay, duration, x, size, icon: Icon, color, rotate, blur, opacity }: any) => (
  <MDiv
    ...
    animate={{ 
      ...
      x: [`${parseFloat(x)}vw`, `${parseFloat(x) + (Math.random() * 15 - 7.5)}vw`]
    }}
    transition={{ 
      duration, 
      repeat: Infinity, 
...
const BotanicalDecor: React.FC = () => {
  const elements = useMemo(() => {
    ...
    const presets = [
      ...
      { color: 'text-green-500', opacity: 0.3, blur: 1 },
      { color: 'text-rose-400', opacity: 0.5, blur: 0 }, // Cánh hoa hồng
      { color: 'text-amber-300', opacity: 0.4, blur: 2 }, // Bào tử nắng
      { color: 'text-lime-300', opacity: 0.2, blur: 4 }, // Lá ở gần (mờ)
    ];
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 select-none">
      {/* Dynamic Floating Garden Particles */}
      {elements.map((el) => (
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-emerald-600/10 rounded-full blur-[150px] animate-pulse" />
      ...
      {/* Subtle Ground Fog/Grass effect */}
